...
Each layer has its own CLAUDE.md with full coding standards and testing requirements:
...
**Styling is scoped CSS, PrimeVue components, and design tokens.**
...
- **Treat `wailsjs/go/models.ts`, `wailsjs/go/main/App.js`, and `wailsjs/go/main/App.d.ts` as generated output.** They're owned by Wails — run `wails build` to regenerate them after adding or changing any Wails-bound Go method or DTO.
- **Treat the live SMB save game file as read-only.** The team transfer tool is the sole exception, and it only ever writes to a copy.
- **Write a test for every feature, however small.** Edge cases in stat calculations and import logic are where bugs live.
- **Accompany every non-trivial bug fix with a test that would have caught it.** "Non-trivial" means: nullable data, domain encoding/decoding, multi-step data flow (import pipeline, migration), conditional branching on domain values, or anything where a wrong assumption caused the original bug.
- **Explain every `//nolint:gocognit` suppression in a comment** describing why decomposition isn't feasible (e.g., `//nolint:gocognit // large switch over domain enum — splitting would obscure the mapping`). A bare suppression becomes invisible debt that never gets revisited — if you find one anywhere in the codebase, flag it before continuing.
- **Wire every new function to its intended caller as part of finishing the task** — a new store method into its service, a new service method into its Wails binding, a new binding into the frontend. Wiring is part of the task, not a follow-up.
- **Delete functions that have no real caller, or whose only callers are in `_test.go` files.** Tests alone don't make code part of the working application. Git preserves history, so there's no need for commented-out blocks, unused exports, or backwards-compatibility stubs kept "just in case."
