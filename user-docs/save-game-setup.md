...
- **Save File**: the `.sav` file for your franchise. It looks specifically for files named `league-<id>.sav` and filters out auxiliary files like `master.sav`, `mugshots-*.sav`, and `season-*.sav`.
...
::: tip
smb-tools supports both Franchise Mode and Season Mode saves.
:::
