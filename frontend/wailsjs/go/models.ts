export namespace main {
	...
	export class HoFPageDTO {
	    items: HoFCandidateDTO[];
	    total: number;
	
	    static createFrom(source: any = {}) {
	        return new HoFPageDTO(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.items = this.convertValues(source["items"], HoFCandidateDTO);
	        this.total = source["total"];
	    }
	...
	export class TeamSeasonMediaGroupDTO {
	    seasonNum: number;
	    teamHistoryId: number;
	    teamName: string;
	    items: MediaItemDTO[];
	    ...
	    static createFrom(source: any = {}) {
	        return new TeamSeasonMediaGroupDTO(source);
	    }
