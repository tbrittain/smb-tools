...
- No `any` types — Biome will error
- No `as unknown as T` casts — fix the source type instead
- No Options API — Composition API with `<script setup>` only
- No Tailwind — ever
- No hand-editing autogenerated files (`wailsjs/go/models.ts`, `wailsjs/go/main/App.js`, `wailsjs/go/main/App.d.ts`) — run `wails build` after any Go binding/DTO changes
